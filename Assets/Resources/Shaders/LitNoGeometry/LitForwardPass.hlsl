...
Varyings LitPassVertex(uint id : SV_VertexID)
{
    ...
    VertexNormalInputs normalInput = GetVertexNormalInputs(normal, float4(0,0,0,0));
    ...
    real sign = 0 * GetOddNegativeScale();
    ...
    OUTPUT_LIGHTMAP_UV(float(0,0), unity_LightmapST, output.staticLightmapUV);
#ifdef DYNAMICLIGHTMAP_ON
    output.dynamicLightmapUV = float2(0,0) * unity_DynamicLightmapST.xy + unity_DynamicLightmapST.zw;
#endif
