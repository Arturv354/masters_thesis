...
{
    Triangle triangleStruct = triangleBuffer[inputMesh.id / 3];
    float3 localPos;
    float3 normal;
    if (inputMesh.id % 3 == 0)
    {
    ...
    {
        inputMesh.tangentOS = float4(t1, 1);
    }
    else
    {
        inputMesh.tangentOS = float4(t2, 1);
    }
